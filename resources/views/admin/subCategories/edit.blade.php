...
<main class="nxl-container">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-4 mb-4">
                                        ...
                                        <select name="store_id" id="store_id" class="form-select form-control @error('store_id') is-invalid @enderror">
                                            ...
                                            @foreach ($stores as $store)
                                                <option value="{{ $store->id }}" {{ old('store_id', $subCategory->store_id) == $store->id ? 'selected' : '' }}>
                                                    {{ $store->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-lg-4 mb-4">
                                        ...
                                        @if($subCategory->image)
                                            <img id="imagePreview" src="{{ $subCategory->image_url }}" alt="Current Image"
                                                class="mt-2" style="max-width:100px;border-radius:4px;">
                                        @else
                                            <img id="imagePreview" src="#" alt="Preview" class="mt-2 d-none"
    ...
    <script>
        ...
        function filterCategories(storeId, keepSelected) {
            var currentSelected = keepSelected || $('#category_id').val();
            ...
            if (keepSelected) {
                $('#category_id').val(keepSelected);
            }
        }

        $(document).ready(function () {
            var initialStore = $('#store_id').val();
            var initialCategory = '{{ old('category_id', $subCategory->category_id) }}';
            if (initialStore) filterCategories(initialStore, initialCategory);

            $('#store_id').on('change', function () {
                filterCategories($(this).val(), null);
            });
        });
    </script>
    @endpush
